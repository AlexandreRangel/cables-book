...
\newcommand{\SectionThreeSpacingAfter}{0.5ex plus 0.3ex minus 0.1ex}  % Spacing after subsections (reduced to minimize gap before op images)
...
%% ------------------------------------------------------------
\newcommand{\OpImageWidthFraction}{0.90}   % Op image width (0.90 = 90% of column width for 2-col layout)
...
\renewenvironment{figure}[1][htbp]{%
  ...
  \ifnum\value{section}=13\relax
    ...
    % For op images immediately following subsubsection headings:
    % - Don't add \par (paragraph break) which causes extra line
    % - Don't add extra vspace - subsubsection spacing handles it
    % This prevents the weird extra line breaks before op images
  \else
    % Regular figures get standard spacing
  ...
  \if@opimage
    % Op image: minimal spacing after (subsubsection spacing handles before)
    \par\vspace{\OpImageSpacing}%
  \else
