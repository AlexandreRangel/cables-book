...
REM Cover page first, then chapters. Replaces .svg with .pdf for XeLaTeX
powershell -ExecutionPolicy Bypass -Command "$files = @('chapters\01-introduction.md', 'chapters\02-getting-started.md', 'chapters\03-2d-graphics.md', 'chapters\04-3d-graphics.md', 'chapters\05-texturing.md', 'chapters\06-shaders-glsl.md', 'chapters\07-javascript-ops.md', 'chapters\08-audio-sound.md', 'chapters\09-animation-timeline.md', 'chapters\10-interfaces.md', 'chapters\11-export-deployment.md', 'chapters\12-video-tutorials.md', 'chapters\13-AllOps.md'); $content = ''; if (Test-Path 'chapters\0-Cover.md') { Write-Host '  Adding: chapters\0-Cover.md'; $content += (Get-Content 'chapters\0-Cover.md' -Raw -Encoding UTF8); $content += [char]10 + [char]10; }; foreach ($f in $files) { if (Test-Path $f) { Write-Host ('  Adding: ' + $f); $content += '\pagebreak' + [char]10 + [char]10; $content += (Get-Content $f -Raw -Encoding UTF8); $content += [char]10 + [char]10; } }; $content = $content -replace '!\[\[([^\]]+)\]\]', '![](C:/Dropbox/Rangel-Vault/Media/image/$1)'; $content = $content -replace '\.svg\)', '.pdf)'; [System.IO.File]::WriteAllText('temp_combined_book.md', $content, [System.Text.Encoding]::UTF8)"
