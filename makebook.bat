...
powershell -ExecutionPolicy Bypass -Command "& { $startTime = Import-Clixml -Path 'timer_start.xml'; $files = @('chapters\01-introduction.md', 'chapters\02-getting-started.md', 'chapters\03-2d-graphics.md', 'chapters\04-3d-graphics.md', 'chapters\05-texturing.md', 'chapters\06-shaders-glsl.md', 'chapters\07-javascript-ops.md', 'chapters\08-audio-sound.md', 'chapters\09-animation-timeline.md', 'chapters\10-interfaces.md', 'chapters\11-export-deployment.md', 'chapters\12-video-tutorials.md'); $section13Files = Get-ChildItem -Path 'chapters' -Filter '13-*.md' | Where-Object { $_.Name -ne '13-_AllOps.md' } | Sort-Object Name | ForEach-Object { $_.FullName }; $content = ''; foreach ($f in $files) { if (Test-Path $f) { $elapsed = (Get-Date) - $startTime; $h = [math]::Floor($elapsed.TotalHours); $m = $elapsed.Minutes; $s = $elapsed.Seconds; Write-Host ('[Elapsed: ' + $h.ToString('00') + ':' + $m.ToString('00') + ':' + $s.ToString('00') + ']') -NoNewline; Write-Host ('  Adding: ' + $f); $content += (Get-Content $f -Raw -Encoding UTF8); $content += [char]10 + [char]10 + '\pagebreak' + [char]10 + [char]10; } }; Write-Host ('  Adding ' + $section13Files.Count + ' section 13 files...'); foreach ($f in $section13Files) { $elapsed = (Get-Date) - $startTime; $h = [math]::Floor($elapsed.TotalHours); $m = $elapsed.Minutes; $s = $elapsed.Seconds; Write-Host ('[Elapsed: ' + $h.ToString('00') + ':' + $m.ToString('00') + ':' + $s.ToString('00') + ']') -NoNewline; Write-Host ('  Adding: ' + $f); $content += (Get-Content $f -Raw -Encoding UTF8); $content += [char]10 + [char]10 + '\pagebreak' + [char]10 + [char]10; }; $content = $content -replace '!\[\[([^\]]+)\]\]', '![](C:/Dropbox/Rangel-Vault/Media/image/$1)'; $content = $content -replace '\.svg\)', '.pdf)'; [System.IO.File]::WriteAllText('temp_combined_book.md', $content, [System.Text.Encoding]::UTF8) }"
